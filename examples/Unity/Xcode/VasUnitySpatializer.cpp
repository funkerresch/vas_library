...
namespace Spatializer
{
        ...
        void LoadHRTF(EffectData *effectData, char *fullpath)
        {
            ...
            if(effectData != NULL)
            {
                ...
                if(!strcmp(fileExtension, "sofa"))
                {
                    ...
                    vas_fir_readSofa((vas_fir *)x, fullpath, ((vas_fir *)x)->left->filter->firSetup);
    ...
    UNITY_AUDIODSP_RESULT UNITY_AUDIODSP_CALLBACK CreateCallback(UnityAudioEffectState* state)
    {
       ...
       // int numberOfReflections = effectdata->reflectionOrder * effectdata->numberOfRays;
        ...
        for(int i = 0; i < 24; i++)
        {
            effectdata->reflection[i] = vas_fir_binaural_new(VAS_VDSP | VAS_BINAURALSETUP_STD | VAS_LOCALFILTER, 256, NULL);
