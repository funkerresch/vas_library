...
namespace Spatializer
{
    ...
    extern "C"
    {
		...
		__declspec(dllexport) void SetDebugFunction( FuncPtr fp )
        {
            Debug = fp;
        }
        
		__declspec(dllexport) void *GetInstance(int id)
        {
            if(Debug != NULL)
                Debug("Trying to find Renderer fuck fuck");
            int i=0;
		...
		__declspec(dllexport) void SetNumberOfRays(EffectData *effectData, int numberOfRays)
        {
            effectData->numberOfRays = numberOfRays;
        }
        
		__declspec(dllexport) void SetReflectionOrder(EffectData *effectData, int reflectionOrder)
        {
            effectData->reflectionOrder = reflectionOrder;
        }
        
		__declspec(dllexport) void LoadHRTF(EffectData *effectData, char *fullpath)
        {
            vas_fir *x;
            if(effectData != NULL)
            {
    ...
    int InternalRegisterEffectDefinition(UnityAudioEffectDefinition& definition)
    {
        ...
        RegisterParameter(definition, "H SDirectivity", "", 0.0f, 360, 80.0f, 1.0f, 1.0f, P_H_SOURCEDIRECTIVITY, "Horizontal Source Directivity");
        RegisterParameter(definition, "H FP Range", "", 0.0f, 360, 40.0f, 1.0f, 1.0f, P_H_FULLPOWERRANGE, "Horizontal Full Power Range");
        RegisterParameter(definition, "V SDirectivity", "", 0.0f, 360, 0.0f, 1.0f, 1.0f, P_V_SOURCEDIRECTIVITY, "Vertical Source Directivity");
        RegisterParameter(definition, "V FP Range", "", 0.0f, 360, 0.0f, 1.0f, 1.0f, P_V_FULLPOWERRANGE, "Vertical Full Power Range");
        ...
        RegisterParameter(definition, "Occ Filter", "", 0.0f, 20000.0f, 20000.0f, 1.0f, 1.0f, P_OCCLUSION_FREQ, "Occlusion Filter Frequency");
