...
public class VasSpatConfigAuto : MonoBehaviour
{
    ...
    private int VAS_REFLECTIONPARAMETERS = 5;
    private Ray ray;
    ...
    void RaytraceReflections(int rayNumber, float azimuth, float elevation)
    {
       // reflectionOrder = Mathf.Clamp(reflectionOrder, 1, reflectionOrder);
        ...
        for (int i = 0; i < reflectionOrder; i++)
        {
            ...
            {
                mySource.SetSpatializerFloat(paramIndex + 4, 0f); // no hit, mute reflection
            ...
            {
                // this must be done with a fadeout and clearing of the delay buffer
                //mySource.SetSpatializerFloat(paramIndex + 4, 1f); // no hit, mute reflection
                //print("mute");
