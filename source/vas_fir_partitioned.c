...
void vas_fir_partitioned_swapReadAndWrite(vas_fir_partitioned *x, int i)
{
    ...
    x->convolutionEngine[i]->left->useSharedInput = !x->convolutionEngine[i]->left->useSharedInput; // was not the original intention but we only need to calculate input fft for every second input
    x->convolutionEngine[i]->right->useSharedInput = !x->convolutionEngine[i]->right->useSharedInput;
}
...
/*
 ...
 After reading the next 4 min size frames at time 1024 the first 1024 frame should be calculated for output, we switch read and write buffers, turn off the input
 fft and use instead the already calculated input fft for the convolution with the next 1024'er filter segment. At the same time (2048) we have collected enough
 samples to start convolution with the first 2048 filter frame..
 */
