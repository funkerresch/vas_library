...
int32_t vas_fir_parallel_instanceCounter;
...
{
    vas_fir_parallel *x = ( vas_fir_parallel * )vas_mem_alloc(sizeof(vas_fir_parallel));
    ...
    {
        vas_convolutionThreadPool = vas_threads_init(numberOfThreads);
        vas_fir_parallel_instanceCounter++;
    }
    
    return x;
}

void vas_fir_partitioned_swapReadAndWrite(vas_fir_parallel *x, int i)
{
...
int vas_fir_parallel_init(vas_fir_parallel *x, int minSegmentSize, int maxSegmentSize, int length)
{
    ...
    x->partitionData = (vas_fir_parallel_data *) vas_mem_alloc(sizeof(vas_fir_parallel_data) * x->convolverCount);
    ...
    vas_fir_parallel_data_set(&x->partitionData[0], minSegmentSize, 0, currentEnd);
    ...
    for(int i = 1; i < x->convolverCount; i++)
    {
        ...
        vas_fir_parallel_data_set(&x->partitionData[i], currentSize, currentStart, currentEnd);
...
void vas_fir_parallel_free(vas_fir_parallel *x)
{
    ...
    vas_mem_free(x);
    vas_fir_parallel_instanceCounter--;
    if(!vas_fir_parallel_instanceCounter)
        vas_threads_destroy1(vas_convolutionThreadPool, x->writeJobQueueLeft);
}
